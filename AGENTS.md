...
- DNS resolved-address validation is ON by default and runs once per request for hostname targets. Add `skipDnsValidation: true` (on `defineProviderExecutors`/`defineProviderProxy`/`createProviderFetch`) ONLY when the egress host is a hardcoded literal fully controlled by the code. NEVER add it when the host comes from credential/user input, when the base URL is a resolver, or when the provider fetches a user-supplied URL — there the DNS check is the SSRF defense, not redundant overhead. Note that this flag is independent of `OOMOL_CONNECT_ALLOW_PRIVATE_NETWORK`: skipping DNS validation takes effect on every deployment, not just ones that opted into private networks.
- One standing exception to the rule above: the local-Docker family (`comfyui`, `firecrawl_local`, `openscad`, `searxng`) pairs `skipDnsValidation: true` with a credential-supplied `baseUrl`. OrbStack resolves `host.docker.internal` to 0.250.250.254, inside the always-blocked 0.0.0.0/8 range that no opt-in can reopen, so these four have no other way to reach their own instance; the residual risk is carried by `custom_credential` values being admin-scoped (`PUT /api/connections`), never reachable from an MCP/runtime-token caller. This exception does NOT generalize — do not copy the pair into a new provider without adding it to this list, and never into one that fetches user-supplied URLs. Two ways out when it becomes worth paying for: teach `skipDnsValidation` to take a host predicate so only the Docker gateway is exempt, or reach the instance over a private LAN/tailnet IP (RFC 1918 and 100.64/10 classify as `private`, which the opt-in already covers).
